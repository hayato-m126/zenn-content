...
0.15.3-1で入った直後には、clockが出るより先にrecordが動いてしまうと、時刻が0(1970-)スタートになってしまっていましたが、ヘルプを見ると、clockが届いてからレコードするように変更が入っていました。
recordの開始タイミングをrecorderの方で調整してくれるようになったのでより便利になっていました。
